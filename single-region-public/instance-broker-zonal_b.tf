resource "aws_network_interface" "interfaces_zone_b" {
  count = var.zonal_broker_count

  private_ips = ["${var.prefix}.${var.zones[1]}.${100 + count.index}"]
...
resource "aws_instance" "brokers_zone_b" {
  count                       = var.zonal_broker_count
  ...
  network_interface {
    network_interface_id = aws_network_interface.interfaces_zone_b[count.index].id
    device_index = 0
  }
