resource "aws_instance" "broker_use1" {
  ...
  associate_public_ip_address = var.broker_public
  key_name                    = var.use1_ec2_public_key_name

  subnet_id                   = var.broker_public ? module.vpc_use1.public_subnets[count.index % 3] : module.vpc_use1.private_subnets[count.index % 3]
  vpc_security_group_ids      = var.broker_public ? [
...
resource "aws_instance" "broker_use2" {
  ...
  subnet_id                   = var.broker_public ? module.vpc_use2.public_subnets[count.index % 3] : module.vpc_use2.private_subnets[count.index % 3]
  vpc_security_group_ids      = var.broker_public ? [
