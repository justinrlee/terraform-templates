...
data "aws_eks_cluster" "r0" {
  name = module.eks_cluster_r0.cluster_id

  provider = aws.r0a
}

data "aws_eks_cluster_auth" "r0" {
  name = module.eks_cluster_r0.cluster_id
...
module "eks_cluster_r0" {
  ...
  cluster_name    = "${var.cluster_name}-${var.regions_short[0]}"
  cluster_version = "1.21"
  subnets = module.vpc_r0s.public_subnets
  ...
  providers = {
    aws = aws.r0a
    kubernetes = kubernetes.eks_r0
  }
}

resource "aws_security_group" "eks_worker_group_r0" {

  name = "eks_worker_group_r0"
  vpc_id      = module.vpc_r0s.vpc_id
...
locals {
  userdata_r0 = <<-EOT
    #!/bin/bash
    set -o xtrace
    /etc/eks/bootstrap.sh ${module.eks_cluster_r0.cluster_id}
  EOT
}


resource "aws_launch_template" "launch_template_r0" {
  name = "${var.cluster_name}-workers-${var.regions_short[0]}"

  # todo: change to map lookup
  image_id      = lookup(var.worker_image_id, var.regions[0])
  ...
  network_interfaces {
    ...
    security_groups              = [module.eks_cluster_r0.worker_security_group_id, aws_security_group.eks_worker_group_r0.id]
  }
  ...
  user_data = base64encode(local.userdata_r0)

  provider = aws.r0a
}


resource "aws_autoscaling_group" "r0_workers" {

  name             = "${var.cluster_name}-region-workers-r0"
  max_size         = 10
  min_size         = 0
  desired_capacity = 2
  ...
  vpc_zone_identifier = module.vpc_r0s.public_subnets

  launch_template {
    id      = aws_launch_template.launch_template_r0.id
    version = "$Latest"
  }

  tag {
    key                 = "Name"
    value               = "${module.eks_cluster_r0.cluster_id}-Region-Node"
    propagate_at_launch = true
  }

  tag {
    value               = "owned"
    key                 = "kubernetes.io/cluster/${module.eks_cluster_r0.cluster_id}"
    propagate_at_launch = true
  }
